...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			if (i < 0)
			{
				MessageBox(hwnd, "Âû íè÷åãî íå âûáðàëè", "Info", MB_OK | MB_ICONINFORMATION);
			}
			else
			{
				CHAR sz_message[SIZE] = {};
				sprintf(sz_message, "Áûë âûáðàí ýëåìåíò ¹ %i, ñî çíà÷åíèåì \"%s\"", i, sz_buffer);
...
//ÍÎÂÎÅ ÎÊÍÎ Â ÊÍÎÏÊÅ ÄÎÁÀÂÈÒÜ ÝËÅÌÅÍÒ:
BOOL CALLBACK DlgProcAddElement(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
